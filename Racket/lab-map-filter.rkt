...
(map  (lambda(fnames lnames) (string-append fnames " " lnames)) fnames lnames)
  )
;;lambda parameters dont have to be the same name
=
...
(define (managers-only lst)
   ...
   (filter (lambda(lst) (equal? (cadddr lst) "Manager")) lst)
    
      ;;or
  ; we could have defined
  (define (manager? emp)
    (equal? (caddd, emp) "Manager"))
  (manager? dilbert)
  (filter manager? '(dilbert nike pgh wally))
